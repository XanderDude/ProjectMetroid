...
func go_to_script(script_path: String, line_number : int = 0) -> void:
	if script_path.ends_with(".cs"):
		# C# scripts are opened via Godot's own dotnet/editor/external_editor
		# mechanism, which edit_resource()+goto_line() can't pass a line to.
		var script := load(script_path)
		plugin.get_editor_interface().edit_script(script, line_number)
		return
...
func get_exec_flags(editor_path : String, script_path : String, line_number : int) -> PackedStringArray:
	...
	var editor_file_name := editor_path.get_file().to_lower()
	if editor_file_name.begins_with("code"): ## VS Code
